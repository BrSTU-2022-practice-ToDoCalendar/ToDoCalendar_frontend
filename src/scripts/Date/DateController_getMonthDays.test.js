...
test('DateController.getMonthDays() Мы получили прямоугольный календаря делящий на 7?', () => {
...
test('DateController.getMonthDays() Первый день это понеделник в кадратном календаре?', () => {
  let real;
...
test('DateController.getMonthDays() Последний день это воскресенье в кадратном календаре?', () => {
  let real;
